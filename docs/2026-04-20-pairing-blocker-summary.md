...
Confirm visibility first on the host addressed by `REMOTE_HOST`:

```bash
ssh "$REMOTE_HOST" 'bluetoothctl --timeout 12 scan on | tee /tmp/bike-scan.out'
ssh "$REMOTE_HOST" "grep -E '00:04:63:BA:64:FC|smart system eBike' /tmp/bike-scan.out"
```

Enable host-side bluetoothd debug logging:

```bash
./scripts/bluetoothd-debug-enable
./scripts/bluetoothd-debug-tail
```

Run a traced Python-path handshake:

```bash
ssh "$REMOTE_HOST" 'tmux new-session -d -s boschpy -n btmon "sudo btmon -w /tmp/boschpy.snoop"'
ssh "$REMOTE_HOST" 'tmux new-window -t boschpy -n shell'
ssh "$REMOTE_HOST" 'tmux send-keys -t boschpy:shell.0 "cd ~/bosch-ble && bluetoothctl remove 00:04:63:BA:64:FC >/dev/null 2>&1 || true && BOSCH_BLE_AGENT_LOG=/tmp/bosch-agent.log uv run bosch-ble-handshake 00:04:63:BA:64:FC /tmp/bosch-live-handshake.log" C-m'
```

Decode the captured pairing request:

```bash
ssh "$REMOTE_HOST" 'sudo btmon -r /tmp/boschpy.snoop > /tmp/boschpy.decode.txt'
ssh "$REMOTE_HOST" "sed -n '/SMP: Pairing Request/,+20p' /tmp/boschpy.decode.txt"
```
